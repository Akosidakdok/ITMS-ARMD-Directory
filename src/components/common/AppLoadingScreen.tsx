...
const MINIMUM_VISIBLE_TIME = 1400;
const EXIT_DURATION = 600;

export const AppLoadingScreen: React.FC<AppLoadingScreenProps> = ({ active, targetProgress, status }) => {
  ...
  useEffect(() => {
    ...
    const completionPause = Math.max(480, MINIMUM_VISIBLE_TIME - elapsed);
    const exitTimer = window.setTimeout(() => setExiting(true), completionPause);
    return () => window.clearTimeout(exitTimer);
  }, [active, displayProgress, mounted]);

  useEffect(() => {
    if (!exiting) return;
    const unmountTimer = window.setTimeout(() => setMounted(false), EXIT_DURATION);
  ...
  return (
    <div className={`pais-splash${exiting ? ' pais-splash--exiting' : ''}`} role="status" aria-live="polite" aria-busy={active}>
