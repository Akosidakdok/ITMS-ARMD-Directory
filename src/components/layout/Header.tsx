...
export const Header: React.FC<HeaderProps> = ({ onMenuClick, onSidebarToggle, sidebarCollapsed = false }) => {
  ...
  return (
    <header role="banner" className="no-print sticky top-0 z-40 flex h-[4.5rem] items-center border-b border-slate-300 bg-[#fcfbf7] px-3 sm:px-5">
      <button
        type="button"
        onClick={onMenuClick}
        className="mr-3 inline-flex h-10 w-10 items-center justify-center rounded border border-slate-300 bg-white text-slate-700 hover:bg-slate-50 lg:hidden"
      ...
      <button
        ...
        className="mr-3 hidden h-10 w-10 shrink-0 items-center justify-center rounded border border-slate-300 bg-white text-slate-700 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 lg:inline-flex"
      ...
      <div className="ml-auto flex items-center gap-2 sm:gap-3">
        <div
          className="hidden items-center gap-2 px-1 text-[10px] font-semibold text-slate-600 md:flex"
          title={systemLabel}
          role="status"
        >
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="flex min-h-10 items-center gap-2 rounded px-1.5 py-1 text-left hover:bg-slate-100 sm:px-2"
          >
            <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded text-[10px] font-bold text-white ${role === 'superadmin' ? 'bg-blue-900 ring-1 ring-amber-400' : 'bg-blue-700'}`}>{initials}</span>
          ...
          {menuOpen && (
            <div role="menu" className="absolute right-0 mt-1 w-56 rounded-md border border-slate-300 bg-white p-1.5 shadow-lg">
